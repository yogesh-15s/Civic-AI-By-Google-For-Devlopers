...
export default function ChatPage() {
    ...
    const loadHistory = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setLang(data.language === 'hi' ? 'hi' : 'en');
        }
      } finally {
  ...
  const persistHistory = async (nextMessages: Message[], nextLang: 'en' | 'hi' = lang) => {
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      const nextMessages = [...conversation, aiMsg];
      ...
      const nextMessages: Message[] = [
        ...conversation,
        {
          id: (Date.now() + 1).toString(),
          role: 'assistant',
          content: 'Connection error. Please check your backend URL and try again.',
          time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        },
      ];
  ...
  if (status === 'loading' || (status === 'authenticated' && loadingHistory)) {
    return (
      <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center' }}>
        <p style={{ color: 'var(--text-secondary)' }}>Loading your chat history...</p>
      </div>
    );
  }
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', background: 'var(--bg-secondary)', borderRadius: '10px', padding: '3px' }}>
          {(['en', 'hi'] as const).map((language) => (
            <button key={language} onClick={() => setLang(language)} style={{
              padding: '6px 14px', borderRadius: '8px', border: 'none', cursor: 'pointer',
              fontWeight: 600, fontSize: '13px', fontFamily: 'Inter, sans-serif',
              transition: 'all 0.2s',
              background: lang === language ? 'var(--bg-card)' : 'transparent',
              color: lang === language ? 'var(--text-primary)' : 'var(--text-secondary)'
            }}>
              {language === 'en' ? 'EN' : 'HI'}
            </button>
          ))}
        ...
        {messages.length > 0 && (
          <button onClick={() => void clearChat()} style={{
            ...
            <div style={{ textAlign: 'center', marginTop: '10vh' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', textAlign: 'left' }}>
                {STARTERS.map((starter, i) => (
                  <button key={i} onClick={() => void sendMessage(starter)} className="card" style={{
